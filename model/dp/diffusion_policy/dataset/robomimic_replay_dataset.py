...
class RobomimicReplayDataset(BaseDataset):
    def __init__(
        ...
        replay_buffer = _convert_robomimic_to_replay(
            store=zarr.MemoryStore(), shape_meta=shape_meta, dataset_path=dataset_path
        )
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        this_normalizer = concatenate_normalizer(
            [
                get_range_normalizer_from_stat(
                    array_to_stats(data_cache["action"][..., :3])
                ),  # pos
                get_identity_normalizer_from_stat(
                    array_to_stats(data_cache["action"][..., 3:-1])
                ),  # rot
                get_range_normalizer_from_stat(
                    array_to_stats(data_cache["action"][..., -1:])
                ),  # gripper
            ]
        )
...
def _convert_robomimic_to_replay(
    store, shape_meta, dataset_path, n_workers=None, max_inflight_tasks=None
):
