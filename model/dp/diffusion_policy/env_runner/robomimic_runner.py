...
class RobomimicRunner(BaseImageRunner):
    ...
    def __init__(
        ...
        self.rot_aa2mat = RotationTransformer(from_rep="axis_angle", to_rep="matrix")
        ...
        self.rot_quat2euler = RotationTransformer(
            from_rep="quaternion", to_rep="euler_angles", to_convention="XYZ"
        )
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                if key in self.shape_meta["obs"]:
                    ...
                    if isinstance(axis, int):
                        axis = [axis]
                    obs_dict[key] = self.rot_quat2euler.forward(obs[key[:-4]])[
                        :, -self.key_horizon[key] :, list(axis)
                    ]

                # solve rel action
                current_pos = copy.copy(obs_dict["robot0_eef_pos"][:, -1:])
                current_rot_mat = copy.copy(
                    self.rot_quat2mat.forward(obs_dict["robot0_eef_quat"][:, -1:])
                )
                ...
                if prev_action is not None:
                    ...
                    fixed_action_prefix = np.concatenate(
                        [action_pos, action_rot, action_gripper], axis=-1
                    )
                ...
                env_action = action_all[
                    :, self.obs_latency_steps : self.obs_latency_steps + self.n_action_steps, :
                ]
            ...
            all_excuted_actions[this_global_slice] = [
                np.concatenate(x) for x in this_excuted_actions
            ]
