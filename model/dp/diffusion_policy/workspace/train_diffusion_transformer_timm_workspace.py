        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                def log_action_mse(step_log, category, pred_action, gt_action):
                    ...
                    step_log[f"{category}_action_mse_error"] = torch.nn.functional.mse_loss(
                ...
                if (self.epoch % cfg.training.sample_every) == 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        ...
                        # 训练采样动作误差，与 UNet workspace 的 Test Loss 语义保持一致。
                        pred_loss = step_log["train_action_mse_error"].item()
                        print("Test Loss: ", pred_loss)

                        if len(val_dataloader) > 0:
                            val_sampling_batch = next(iter(val_dataloader))
                            batch = dict_apply(
                                val_sampling_batch, lambda x: x.to(device, non_blocking=True)
                            )
                            gt_action = batch["action"]
                            pred_action = policy.predict_action(batch["obs"])["action_pred"]
