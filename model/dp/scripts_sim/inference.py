...
from umi.common.pose_util import mat_to_pose, mat_to_pose10d, pose10d_to_mat, pose_to_mat
...
def main(rgb_msg, joint_msg, ee_pose_msg):
    ...
    if len(action_buffer) != 0:
        ...
        for i in range(n_action_steps):
            # 将相对位姿转为世界坐标下的绝对位姿
            abs_pose = relative_to_absolute_pose(cur_pose=cur_ee_pose, target_rel_pos=actions[i])
            if i == 0:
...
node = RgbJointEePoseActionNode(process_fn=main)
