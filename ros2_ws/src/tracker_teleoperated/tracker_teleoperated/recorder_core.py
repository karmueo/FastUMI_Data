...
_FRAME_HEADER = struct.Struct("<dQ")


class CameraFrameSpool:
    """按时间顺序缓存 JPEG，超过阈值后自动转入临时文件。"""
    ...
    def append(self, timestamp: float, jpeg: bytes) -> None:
        """写入一帧时间戳和 JPEG 内容。"""
        self._file.write(_FRAME_HEADER.pack(timestamp, len(jpeg)))
    ...
    def __iter__(self) -> Iterator[tuple[float, bytes]]:
        ...
        try:
            for _ in range(self._count):
                ...
                timestamp, size = _FRAME_HEADER.unpack(header)
                jpeg = self._file.read(size)
                if len(jpeg) != size:
                    raise IOError("相机缓存 JPEG 不完整")
...
class RecordingSession:
    ...
    def append_image(self, generation: int, timestamp: float, jpeg: bytes) -> None:
        ...
        with self._lock:
            ...
            if episode is None or not self.started_at <= timestamp <= (
                ...
                return
            episode.images.append(timestamp, jpeg)
