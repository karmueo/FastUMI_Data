...
`vive_tracker` 通过 OpenVR 读取一台指定 VIVE Tracker，在 ROS 2 中发布实时位姿、有限长度轨迹、全局静态 TF 和 Tracker 动态 TF，并提供同时启动节点与 RViz2 的 launch 文件。
...
| 类型 | 名称 | 消息或变换 |
| --- | --- | --- |
| Topic | `/vive_tracker/pose` | `geometry_msgs/msg/PoseStamped` 当前有效位姿 |
| Topic | `/vive_tracker/path` | `nav_msgs/msg/Path` 最近的有效位姿轨迹 |
| TF | `/tf_static` | `steamvr_tracking` 到 `steamvr_tracking_ros` 的同原点静态轴变换 |
| TF | `/tf` | `steamvr_tracking_ros` 到 `vive_tracker` 的动态变换 |
...
| `parent_frame` | `steamvr_tracking_ros` | Pose、Path 和动态 TF 使用的新全局坐标系 |
| `child_frame` | `vive_tracker` | TF 的 Tracker 子坐标系 |
| `max_path_points` | `3000` | 轨迹保留的最大点数，必须为正整数 |

节点只发布序列号匹配且连接、位姿均有效的采样。设备缺失或跟踪无效时，节点每秒最多输出一次告警，并保留已经发布的轨迹，不会重复发布最后一帧。

### FastUMI 数采接口边界

本包为 FastUMI 提供 ROS 2 `PoseStamped` 位姿源。当前仓库根目录的 `data_collection.py` 仍使用 ROS 1，并订阅 `nav_msgs/Odometry`，因此不会直接订阅本包话题。后续接入数采时，需要将采集流程迁移到 ROS 2，或者增加明确的消息与 ROS 版本适配层；本包不会额外发布兼容旧脚本的 `Odometry`。
...
打开另一个已经加载 ROS 2 和当前工作区环境的终端，录制节点发布的四个话题：
...
ros2 bag record \
  ...
  --topics /vive_tracker/pose /vive_tracker/path /tf /tf_static
```
...
`/vive_tracker/path` 会在每次有效采样时发布一条包含历史轨迹的完整消息，因此长时间录制时占用空间较大。可以降低 `max_path_points`；如果后续不需要直接显示历史轨迹，也可以只录制 `/vive_tracker/pose`、`/tf` 和 `/tf_static`。
...
因此 TF 树为 `steamvr_tracking → steamvr_tracking_ros → vive_tracker`。前一级是同原点静态轴变换，后一级是 Tracker 实时动态位姿；两级组合后仍等于 OpenVR 原始 Tracker 位姿。
...
ROS 2 节点在 `steamvr_tracking_ros` 中表达 Pose 和 Path，同时保留 `vive_tracker` 的原始局部轴。节点当前不提供交互式相对零点、安装外参或多 Tracker 自动发布。

旧 bag（例如 `/tmp/vive_tracker_session`）没有新的 `/tf_static` 和 `steamvr_tracking_ros` 帧，直接使用新版 RViz 配置回放时不会自动转换。需要新坐标树时应重新录制，或者对旧 bag 进行离线转换。
...
ros2 run tf2_ros tf2_echo steamvr_tracking_ros vive_tracker
```
